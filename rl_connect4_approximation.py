...
from rl_connect4 import RandomAgent, Environment, Connect4Board, QLearningControl

class Linear_QNet(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        super().__init__()
        self.linear1 = nn.Linear(42, 200)
        self.linear2 = nn.Linear(200, 200)
        self.linear3 = nn.Linear(200, 200)
        self.linear4 = nn.Linear(200, 7)
        # self.linear3 = nn.Linear(5012, 7)
...
class QLearningApproximateControl(BaseControlAgent):
    ...
    def __init__(self, epsilon = 0.1, alpha = 0.1, discount_factor = 1, identifier = 1, initial_state_action_value = 0.2, use_target_network = True, tau=0.005):
       ...
       self._model = Linear_QNet(42, 2048, 7)
       self._optimizer = optim.Adam(self._model.parameters())
       self._target_network = Linear_QNet(42, 2048, 7)
       self._tau = tau
       self._criterion = nn.MSELoss()
       self._steps = 1
       self._use_target_network = use_target_network
       self._last_td_error = 1
    ...
    def take_action(self, board):
        ...
        if np.random.uniform() < self._epsilon/self._num_episodes:
            ...
            # print(prediction)
            ordered_best_actions = torch.topk(prediction.flatten(), 7).indices
...
def play_interactive(env, trained_agent):
    ...
    while not terminated:
        ...
        print("-------------------------")
        if terminated:
            trained_agent.first_step(env._current_state)
            break
        action = trained_agent.first_step(env._current_state)
        reward, terminated, env._current_state = env._take_action(action, trained_agent.identifier)
        env._current_state.print()
